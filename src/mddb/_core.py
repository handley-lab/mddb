...
class MDDB:
    ...
    def create(self, yaml: dict, body: str = "", *, relpath: str | None = None, rationale: str) -> Card:
        ...
        try:
            _index.insert_card(self.conn, card, relpath)
        except Exception:
            self._drop_cache()
            raise
        return card
    ...
    def read(self, card_id: str) -> Card:
        return Card.from_file(self.root / _index.lookup_relpath(self.conn, card_id))

    def update(self, card: Card, *, rationale: str) -> Card:
        relpath = _index.lookup_relpath(self.conn, card.id)
        self._write_atomic(self.root / relpath, str(card))
        self._git("add", "--", relpath)
        self._git("commit", "-m", rationale)
        try:
            _index.update_card(self.conn, card, relpath)
        except Exception:
            self._drop_cache()
            raise
        return card
    ...
    def delete(self, card_id: str, *, rationale: str) -> None:
        relpath = _index.lookup_relpath(self.conn, card_id)
        self._git("rm", "--", relpath)
        self._git("commit", "-m", rationale)
        try:
            _index.delete_card(self.conn, card_id)
        except Exception:
            self._drop_cache()
            raise

    def move(self, card_id: str, new_relpath: str, *, rationale: str) -> None:
        old = _index.lookup_relpath(self.conn, card_id)
        ...
        self._git("commit", "-m", rationale)
        try:
            _index.move_card(self.conn, card_id, new_relpath)
        except Exception:
            self._drop_cache()
            raise
