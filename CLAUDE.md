...
One stateless server serves **many** decks: every tool takes `deck` (an absolute path to an mddb root) as its first arg and opens `mddb.MDDB(deck)` per call — no env var, no module-level state. `deck` (and `blob_path`) can address any path the server process can read/write; single-tenant agent, not guarded. Both tools open via a `_open(deck)` helper that **checks the path is a deck** (a `.git` dir is present) and raises a clear error otherwise — boundary error-translation, so a typo'd/non-deck path errors instead of reading empty (reads) or half-writing a card then failing at `git` (writes). The core's no-`is-a-deck`-check rule is about the chicken-and-egg with `MDDB.init`; that doesn't apply at this layer, which never bootstraps through `_open`.
