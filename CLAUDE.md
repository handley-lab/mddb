...
- **No migration code.** Pre-alpha. No backwards-compatibility shims. If the on-disk layout changes, the change is the migration: bump the schema-version row in `meta`, and the next open rebuilds the index from the Git-tracked cards at the locked HEAD. Don't write code to read old schemas.
...
**Ownership is exact-stem, filesystem-discovered.** A card at `D/S.md` owns the single non-`.md`, nonempty-suffix file in `D` whose `Path.stem == S`. So `notes.extra.pdf` (stem `notes.extra`) belongs to `notes.extra.md`, never to `notes.md`. Two such siblings is drift the cache can't represent → `ValueError` (at read, rebuild, and the lifecycle plan phase). Rebuild enumerates tracked cards from Git at the locked HEAD, then discovers each card's blob from its worktree siblings; creating a tracked `.md` beside a pre-existing same-stem binary therefore **adopts** it (describe-an-existing-file is first-class).
...
All mutation flows through `db.editor()`. The commit phase, on clean `__exit__`, runs under an advisory exclusive `fcntl.flock` on the read-only `.git` directory descriptor (`_index.deck_lock`) that serialises mddb writers:
...
CREATE TABLE entries(rowid INTEGER PRIMARY KEY, id TEXT UNIQUE, relpath TEXT UNIQUE, title TEXT, summary TEXT, blob_relpath TEXT, first_commit TEXT, yaml_text TEXT, body TEXT);
...
`title` and `summary` are promoted to dedicated nullable columns on `entries` because they are part of the substrate filing/disclosure vocabulary; `db.list()` reads them directly. `blob_relpath` is likewise a dedicated nullable column — a derived cache of `blob_on_disk(card)` (see "Blob cards"), refreshed on every create/move/update and rebuilt by the cache rebuild, so `db.list()` reports blob presence in bulk without a per-card filesystem scan. The substrate computes that column from disk, not from YAML, so the derived column is never an `entry_fields` row. `first_commit` follows the same derived-column pattern from the other truth source: the author date of the first commit in the card's surviving lineage (**renames followed, copies not** — a copied-then-edited card is a new card with its own birth date, so `git log --follow`'s copy-chasing is deliberately not reproduced), stored as canonical UTC ISO `YYYY-MM-DDTHH:MM:SS+00:00` (lexicographic order == chronological order), `NOT NULL` — derived from the deck's git history at rebuild by a single `git log --topo-order --reverse --name-status -M` pass replayed into a per-path origin map (add starts a lineage, rename carries it, delete ends it); only a HEAD path the linear replay can't place (a delete linearised ahead of the merge that preserved the file, or a rename whose source the replay lost) falls back to per-path `git log --follow`, and a lost rename source is left unresolved for that fallback rather than dated from the rename — and stamped from the fresh commit on editor creates. It is substrate provenance, never a YAML field; untracked Markdown is not database input. `entry_fields` indexes every *other* top-level scalar and list-of-scalars from `card.yaml` (notably `tags`) but skips `title`/`summary` (their own dedicated columns).
...
One stateless server serves **many** decks: every tool takes `deck` (an absolute path to an mddb root) as its first arg and opens `mddb.MDDB(deck)` per call — no env var, no module-level state. Unix access to `deck` is the authority boundary. Both tools open via a `_open(deck)` helper that **checks the path is a deck** (a `.git` dir is present) and raises a clear error otherwise — boundary error-translation, so a typo'd/non-deck path errors instead of reading empty (reads) or half-writing a card then failing at `git` (writes). The core's no-`is-a-deck`-check rule is about the chicken-and-egg with `MDDB.init`; the privileged MCP never bootstraps decks.
...
- `editor(deck, rationale, ops, base="")` — `ops` is a JSON-array string run as ONE `db.editor()` block → one commit. `base` (thread the `base` from the `read` whose data you're editing) → `db.editor(base=...)`: cross-call conflict-safety, so a concurrent agent's commit raises `ConflictError` (→ `ToolError`; re-read and retry) instead of clobbering. Op shapes mirror `_Editor` (`create`/`update`/`delete`/`move`/`edit`); optional fields are membership-gated (no `dict.get`); `update` re-reads via `e.read` to dodge the stale-snapshot footgun. Any `id` may be `"$prev[N]"` to reference the id returned by the Nth earlier op. Blob creation carries caller-owned bytes as `blob_base64` and `blob_ext`; `blob_path` and `init` are rejected because the privileged process must not read its private server-local files or create authority-bearing repositories.
...
- Blob discovery is filesystem-based. A manually-dropped binary beside a tracked card *is* its blob on the next `db.read`/rebuild — and `editor.move`/`delete` will try to `git mv`/`git rm` it; if it's untracked, the lifecycle plan phase raises `ValueError` before any mutation. `git add` it (or write it via `editor.create(blob=...)`) first.
